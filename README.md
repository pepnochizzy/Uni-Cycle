...
### What went well/Lessens learned

It was a really great project to work together. The planning stage was really well structured and we were able to fulfill all of the necessary requirements. Each morning we divided up the tasks based on our standup and updated the Trello board. I always felt like I could ask for help if I was stuck and I really enjoyed the teamwork, especially when coding together and trying to figure out new topics. We each played to our strengths and we were able to learn from each other. For me it was especially helpful when it comes to styling.

### Image Buckets

We wanted users to be able to upload their own images from their local machine. I did some previous research on Storage Buckets in Supabase for a previous assignment but wasn't able to implement it then. During the planning sessions I did some additional research and managed to set up a storage bucket within supabase. Important here is that while the bucket is public, I still had to add policies to Select and Insert to allow the SubmitForm to upload and the marketplace to display the images alongside the post.
...
We now have a working filter and sorting function but there are probably easier ways to accomplish this.
